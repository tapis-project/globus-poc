...
def main():
    ...
    print("Activating connect personal endpoint:", ENDPOINT_ID_DST)
    try:
        transfer_client.endpoint_autoactivate(ENDPOINT_ID_DST)
    except GlobusAPIError as ex:
        print(ex)
        if ex.http_status == 401:
            sys.exit("Error activating personal connect endpoint. Most likely refresh token has expired.")
        else:
            raise ex

    # activate globus tutorial endpoint
    print("Activating tutorial endpoint:", ENDPOINT_ID_SRC)
    try:
        transfer_client.endpoint_autoactivate(ENDPOINT_ID_SRC)
    except GlobusAPIError as ex:
        print(ex)
        if ex.http_status == 401:
            sys.exit("Error activating tutorial endpoint. Most likely refresh token has expired.")
        else:
            raise ex
    ...
    ep_personal = transfer_client.get_endpoint(ENDPOINT_ID_DST)
    # EP_DIR = "/~/data/globus"
    ep_personal_dir = ep_personal["default_directory"]

    # list files for connect personal endpoint
    print("============================================================================================")
    print("Listing files for personal endpoint:", ENDPOINT_ID_DST, " path: ", ep_personal_dir)
    for fentry in transfer_client.operation_ls(ENDPOINT_ID_DST, path=ep_personal_dir):
        print("file:", json.dumps(fentry, indent=2, sort_keys=True))
    print("============================================================================================")

    # list files for tutorial endpoint
    ep_tut_dir = "/share/godata"
    print("============================================================================================")
    print("Listing files for tutorial endpoint:", ENDPOINT_ID_SRC, " path:", ep_tut_dir)
    for fentry in transfer_client.operation_ls(ENDPOINT_ID_SRC, path=ep_tut_dir):
        print("file:", json.dumps(fentry, indent=2, sort_keys=True))
    print("============================================================================================")

    # make a new dir on personal endpoint
    dst_dir = ep_personal_dir + "/new_dir"
    print("============================================================================================")
    print("Creating new directory on personal endpoint. Dir:", dst_dir)
    transfer_client.operation_mkdir(ENDPOINT_ID_DST, path=dst_dir)
    print("============================================================================================")

    # Now transfer files from tutorial endpoint to new directory on personal endpoint
    # Build up data structure containing info needed for txfr
    src_dir = "/share/godata"
    f1 = "file1.txt"
    f2 = "file2.txt"
    f3 = "file3.txt"
    ...
    # Start the txfr
    print("============================================================================================")
    print("Transferring files to connect personal dir:", dst_dir, " from tutorial ep dir:", src_dir)
    ...
    # rename a file on personal endpoint
    f1a = "file1a.txt"
    old_path = dst_dir + "/" + f1
    new_path = dst_dir + "/" + f1a
    print("============================================================================================")
    print("Renaming file on personal endpoint. Old path:", old_path + " New path:", new_path)
    transfer_client.operation_rename(ENDPOINT_ID_DST, oldpath=old_path, newpath=new_path)
    print("============================================================================================")

    # delete a file on personal endpoint
    print("============================================================================================")
    print("Deleting file3 on personal endpoint. Path:", new_path)
    ...
    # Recursively list files on personal endpoint.
    # See https://github.com/globus/globus-sdk-python/blob/main/docs/examples/recursive_ls.rst
    dir_queue = deque()
    dir_queue.append((ep_personal_dir, "", 0))
    max_depth = 4
    print("Recursive listing on personal endpoint. Directory: {} Max depth: {}".format(ep_personal_dir, max_depth))
